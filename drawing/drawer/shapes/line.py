...
class LineShape(BaseShape, DrawInCanvas):
    ...
    def fills_in_canvas(self):
        return (
            ...
            (self.y2 >= 0 and self.y2 <= self.canvas.height) and
            ((self.x1 == self.x2) or (self.y1 == self.y2)) # only horizontal or vertical lines allowed
        )
    ...
    def draw_vertical_line(self):
        ...
        self.canvas.canvas[row_range_init:row_range_end, column] = LINE_SHAPE_CHARACTER

    def draw_horizontal_line(self):
        row = self.y1 - 1
        column_range_init = self.x1 - 1
        column_range_end = self.x2
        self.canvas.canvas[row][column_range_init:column_range_end] = LINE_SHAPE_CHARACTER
