...
class DrawInCanvas(ABC):
    ...
    def draw_in_canvas(self, canvas):
        ...
        if not self.fills_in_canvas():
            message = self.shape_name + ' can\'t be drew, it doesn\'t fill in canvas'
...
class BaseShape(ABC):
    ...
    def validate(self, strokes):
        ...
        if not strokes_count == self.shape_strokes:
            self.invalid_message = 'Strokes should be ' + str(self.shape_strokes) +', given ' + str(strokes_count)
            return False
        ...
        if self.shape_strokes == 4 and (strokes[0] == strokes[2] and strokes[1] == strokes[3]):
            self.invalid_message = self.shape_name + ' can\'t be drew, start and end stroke points are the same'
            return False
        return True

    def build(self, strokes):
        if not self.validate(strokes):
            print('the given shape data for ' + self.shape_name + ' is invalid')
